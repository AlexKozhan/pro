...
class LoginPage(BasePage):
    ...
    def __init__(self, page: Page):
        super().__init__(page)
        self.USERNAME_INPUT = page.locator(LoginPageLocators.
                                           USERNAME_INPUT)
        self.PASSWORD_INPUT = page.locator(LoginPageLocators.
                                           PASSWORD_INPUT)
    ...
    def login(self, username, password):
        ...
        if not LOGIN_PAGE_URL:
            raise ValueError("LOGIN_PAGE_URL is not "
                             "defined or is empty!")
    ...
    def is_error_message_visible(self) -> bool:
        """Check if error message appears (for invalid
        login attempts)."""
        try:
    ...
    def is_login_button_visible(self) -> bool:
        """Check if login button is still visible (after
         logout)."""
        return self.LOGIN_BUTTON.is_visible()

    @allure.step("Verifying login page is loaded")
    def is_login_page_loaded(self) -> bool:
        """Check if the login page is loaded (checking the
        presence of the login button)."""
        return self.LOGIN_BUTTON.is_visible()
